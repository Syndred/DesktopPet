...
- `docs/lingjing-pc-config-usage.md`
...
## 2.3 v2.25 热修（联机接受不进战斗）

1. 对战申请“接受”流程增加超时保护：
   - 接受前等待“出战灵宠更新”最多约 `1.8s`，避免被历史异步任务卡住。
2. 自动入房策略增强：
   - 当检测到本地仍占用旧房间时，会先静默离开旧房间，再自动加入新匹配房间。
   - 避免“收到已接受申请但一直不进对战”的卡死状态。
3. 失败可见性增强：
   - 接受流程异常现在会写入可读日志，便于定位问题。
4. 版本号：
   - 根包与 PC 运行时版本更新为 `0.1.2`（联机接受流程热修包）。

## 2.4 v2.26 热修（登录后反复自动连接历史房间）

1. 自动入房轮询去抖与防死循环：
   - 自动加入失败后，本次会话不再对同一请求无限重试，避免登录后刷屏“正在自动加入”。
2. 历史请求过期保护：
   - 对过旧的 `accepted` 请求自动跳过（不再自动入房），避免历史房间号干扰当前流程。
3. 版本号：
   - 根包与 PC 运行时版本更新为 `0.1.3`（自动连接死循环修复包）。

## 2.5 v2.27 体验更新（图标/版本历史/文档瘦身）

1. 托盘与程序图标：
   - 托盘图标改为文件资源加载（`apps/pc-app/runtime/assets/icons/tray.png`）。
   - 主窗口/登录窗口补充图标路径（`app.ico`），修复“透明图标/无图标”问题。
   - Windows 打包配置补齐图标：
     - `scripts/release/electron-builder.pc.json` 增加 `icon` 与 `win.icon`。
2. 托盘右键菜单：
   - 菜单文本改为中英自适应（按系统 locale），中文环境不再显示英文菜单项。
3. 版本历史查看：
   - 版本号已并入左上角标题（`灵境` 后面），不再单独占一行。
   - 点击标题后的版本号可弹出“版本更新历史”卡片。
   - 支持中英双语内容，并自动高亮“当前版本”。
4. 模型展示升级（猫狗主题试接入）：
   - 新增并启用 `Shiba Inu / Husky / Cat(A) / Cat(B)` 四个模型（CC0）。
   - 默认仓库、联机元素映射、附近灵宠模型映射已切换到上述猫狗模型。
   - 模型来源清单：`apps/pc-app/runtime/assets/models/catalog.json`。
5. 文档与瘦身：
   - 新增模型来源与导入规范：`docs/model-sourcing-and-import.md`。
   - 新增目录/文件用途地图：`docs/project-structure-and-file-map.md`。
   - 删除无引用临时文件：`.tmp-electron-check.cjs`。
6. 版本号：
   - 根包与 PC 运行时版本更新为 `0.1.4`。

## 2.6 v2.28 展示优化（标题版本位 + 猫狗模型试接入）

1. 版本号展示位调整：
   - 版本号已移动到左上角标题 `灵境` 后方（例如 `灵境 v0.1.5`）。
   - 去除版本单独占行，保持头部更紧凑。
   - 点击该版本号仍可弹出版本历史。
2. 模型展示升级（猫狗主题）：
   - 新增并启用：`Shiba Inu`、`Husky`、`Cat(A)`、`Cat(B)`。
   - 默认仓库、联机元素映射、附近灵宠生成模型已切换到新模型集。
...
## 2.11 v2.33 属性技能与大招规则重做

1. 五行属性技能调整：
   - 金（易伤）：目标受到伤害倍率改为 `1.5x`（可叠层）。
   - 木（寄生）：寄生造成伤害时会为施加者回血。
   - 水（冰封）：命中时立即清除目标当前 `50%` 怒气（例如 `50 -> 25`）。
   - 火（灼烧）：每回合扣除目标当前生命的 `8%`（可叠层）。
   - 土（钝化）：目标造成的伤害减半（可叠层）。
2. 持续与叠加：
   - 灼烧/寄生/易伤/钝化持续 `3` 回合并支持叠加；冰封为即时生效（命中即削减怒气）。
3. 大招统一规则：
   - 双倍伤害 + 附带属性效果 + 固定回复 `20` 点生命。
4. 结算一致性：
   - 本地 `battle-runtime` 与云端 `duel-online` Edge Function 已同步同一套规则，避免单机/联机结算差异。
5. 版本号：
   - 根包与 PC 包版本更新为 `0.1.8`。
